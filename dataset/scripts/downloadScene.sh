...
dataPath=${2-panoptic}
numVGAViews=${3-0} # Specify nbr of VGA views to donwload. Up to 480
numHDViews=${4-0} # Specify nbr of HD views to donwload. Up to 31
N=${5-1}         # Specify sampling frequency (saves every N:th)
...
fullDatasetName="$dataPath/$datasetName"
mkdir $fullDatasetName
mainPath=$PWD
cd $fullDatasetName
otherPath=$PWD
...
$WGET $mO hdPose3d_stage1.tar http://domedb.perception.cs.cmu.edu/webdata/dataset/$datasetName/hdPose3d_stage1.tar || rm -v hdPose3d_stage1.tar
# Download 3D pose reconstruction results (MPI version, by vga index)
$WGET $mO vgaPose3d_stage1.tar http://domedb.perception.cs.cmu.edu/webdata/dataset/$datasetName/vgaPose3d_stage1.tar || rm -v vgaPose3d_stage1.tar
